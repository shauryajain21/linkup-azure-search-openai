...
class ChatReadRetrieveReadApproach(Approach):
    ...
    async def run_search_approach(
        ...
        # STEP 2.5: Query Linkup API based on dual-trigger logic:
        # 1. Keywords detected in query (explicit web search request)
        # 2. Document search results are insufficient (no results meet score threshold)
        ...
        # Add Linkup thought step showing trigger details
        if use_linkup_fallback and self.linkup_client:
            thoughts.append(
                ThoughtStep(
                    "Linkup web search",
